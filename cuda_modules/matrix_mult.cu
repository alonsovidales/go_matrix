...
// CUDA Kernel
__global__ void matrixMul(double* C, double* A, double* B, int wA, int wB, int resW, int resH, int resultSize, int matrixSplits)
{
		...
		for (int by = 0; by < matrixSplits; by++) {
			...
			if (resultPos < resultSize) {
				...
				for (int i = 0; i < wA; ++i)
				{
					...
					if (x == 29 && y == 409) {
						printf("XY %d - %d : Mul: %d %d\n", x, y, A[y * wA + i], B[i * wB + x]);
					}
				}
				...
				if (x == 29 && y == 409) {
					printf("XY %d - %d : %d / %d Val: %d\n", x, y, resultPos, resultSize, value);
				}
